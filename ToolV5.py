...
class SazSummaryTool(BaseTool):
    ...
    def _run(
        ...
        try:
            ...
            print(f"[SazSummaryTool] Generated {len(summary_files)} file(s).")

            uploaded_urls: list[str] = []
            uploaded_summary: list[str] = []
            uploaded_corr: list[str]    = []
            uploaded_cross: list[str]   = []

            for rel_path, content_bytes in sorted(summary_files.items()):
                rp  = f"{folder_name}/{rel_path}"
                msg = f"Add SAZ output: {Path(rel_path).name}"
                ...
                if "_correlations.txt" in name and name.startswith("_cross"):
                    uploaded_cross.append(f"  • {url}")
                elif "_correlations.txt" in name:
                    uploaded_corr.append(f"  • {url}")
                else:
                    uploaded_summary.append(f"  • {url}")

            result_lines = [
                f"Successfully processed and uploaded {len(uploaded_urls)} file(s).",
                f"Repository : {info['owner']}/{info['repo']} → {folder_name}/",
                "",
            ]
            if uploaded_summary:
                result_lines += ["Scenario Summaries:"] + uploaded_summary + [""]
            if uploaded_corr:
                result_lines += ["Combined Correlations Table:"] + uploaded_corr + [""]
            if uploaded_cross:
                result_lines += ["Cross-Group Correlations:"] + uploaded_cross + [""]
            return "\n".join(result_lines)
...
def _correlate_in_memory(sessions1: list, sessions2: list) -> tuple:
    """
    In-memory correlation.
    Returns (group_results, cross_corrs):
      group_results – list of (group_name, dynamic_fields, correlations)
      cross_corrs   – list of cross-group correlation records
    """
    cfg     = Config()
    ...
    for grp in all_groups:
        ...
        aspnet         = find_aspnet_form_correlations(g1, correlations + structural)
        group_results.append((grp, dynamic_fields, correlations + structural + aspnet))

    # Cross-group pass: tokens that flow across group boundaries
    cross_corrs = find_cross_group_correlations(sessions1, group_results, cfg)

    return group_results, cross_corrs


def _build_cross_group_txt(stem: str, cross_corrs: list) -> str:
    """
    Render cross-group correlations as a plain-text tabular report
    (two tables — mirrors _build_combined_correlations_table style).

    TABLE 1 — Unique cross-group variables
      Columns: # | Variable | Captured Value | Source Group |
               Src Sess | Src Method | Source URL | Response Field | Extractor Hint

    TABLE 2 — Cross-group destination map
      Columns: Variable | Source Group | Dst Group | Dst Sess |
               Dst Method | Destination URL | Destination Field
    """
    if not cross_corrs:
        return (
            f"=== CROSS-GROUP CORRELATIONS : {stem} ===\n"
            f"Generated : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
            "  No cross-group correlations detected.\n"
        )

    def _row(*cells, widths: dict) -> str:
        return "  " + "  |  ".join(
            str(c)[:w].ljust(w) for c, w in zip(cells, widths.values())
        )

    lines = [
        f"=== CROSS-GROUP CORRELATIONS : {stem} ===",
        f"Generated              : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        f"Cross-group entries    : {len(cross_corrs)}",
        f"Groups affected        : {', '.join(sorted({c['source_group'] for c in cross_corrs} | {g for c in cross_corrs for g in c.get('cross_groups', [])}))}",
        "",
    ]

    # ── TABLE 1 : Cross-group variables ──────────────────────────────────────
    C1 = {
        "#":             4,
        "Variable":     28,
        "Value":        36,
    ...
    lines += [
        "=" * 130,
        "  TABLE 1 — CROSS-GROUP VARIABLES  (tokens that cross group boundaries)",
        "=" * 130,
        "",
        _row("#", "Variable", "Captured Value", "Source Group", "Sess",
             "Method", "Source URL", "Resp Field", "Extractor Hint", widths=C1),
        "  " + "-+-".join("-" * w for w in C1.values()),
    ]

    for idx, c in enumerate(cross_corrs, 1):
        src  = c.get("source", {})
        hint = _extractor_hint(
            c.get("extraction_note", ""),
            src.get("field", ""),
            _derive_var_name(c.get("extraction_note", ""), c.get("dynamic_field_name", "")),
        )
        var_name = _derive_var_name(c.get("extraction_note", ""),
                                    c.get("dynamic_field_name", ""))
        lines.append(_row(
            str(idx),
            f"${{{var_name}}}",
            str(c.get("token_value", ""))[:36],
            c.get("source_group", "?"),
            str(src.get("session_id", "?")),
            str(src.get("method", "?")),
            str(src.get("url", ""))[:52],
            str(src.get("field", ""))[:30],
            hint[:56],
            widths=C1,
        ))
    ...
    C2 = {
        ...
        "Dst Method":       10,
        "Destination URL":  58,
        "Destination Field": 38,
    }
    lines += [
        "=" * 130,
        "  TABLE 2 — CROSS-GROUP DESTINATION MAP  (where each variable is consumed)",
        "=" * 130,
        "",
        _row("Variable", "Source Group", "Dst Group", "Dst Sess",
             "Dst Method", "Destination URL", "Destination Field", widths=C2),
        "  " + "-+-".join("-" * w for w in C2.values()),
    ]

    for c in cross_corrs:
        src_grp  = c.get("source_group", "?")
        var_name = _derive_var_name(c.get("extraction_note", ""),
                                    c.get("dynamic_field_name", ""))
        for dest in c.get("destinations", []):
            dst_grp = dest.get("group", "?")
            if dst_grp == src_grp:
                continue   # only show cross-group destinations
            lines.append(_row(
                f"${{{var_name}}}",
                src_grp,
                dst_grp,
                str(dest.get("session_id", "?")),
                str(dest.get("method", "?")),
                str(dest.get("url", ""))[:58],
                str(dest.get("field", ""))[:38],
                widths=C2,
            ))
...
def _run_pipeline_in_memory(
    ...
    Returns {relative_path_str: content_bytes} for:
      • *_summary.txt              – one per group
      • {stem}_correlations.txt    – combined correlations table for the scenario
      • _cross_group_correlations.txt – tokens that flow across group boundaries
    """
    ...
    # Stage 3: Correlation (in memory) — includes cross-group pass
    print("[Pipeline] Stage 3 – Correlating …")
    group_results, cross_corrs = _correlate_in_memory(sessions1, sessions2)
    corr_map = _build_correlations_map(group_results)
    print(f"  Per-group correlations  : {sum(len(v.get('correlations',[])) for v in corr_map.values())}")
    print(f"  Cross-group correlations: {len(cross_corrs)}")
    ...
    # Only generate summaries for the first SAZ file (stem1/groups1).
    # The second SAZ file is used solely for correlation/dynamic-value analysis.
    tasks = []
    for idx, (group_name, grp_sessions) in enumerate(groups1.items(), 1):
        tasks.append((stem1, idx, group_name, grp_sessions))
    ...
    # Stage 5: Combined correlations table (one file at scenario level)
    print("[Pipeline] Stage 5 – Building correlation tables …")
    all_corr_dicts = list(corr_map.values())   # list of {group, dynamic_variables, correlations}
    if all_corr_dicts:
        corr_table_txt  = _build_combined_correlations_table(stem1, all_corr_dicts)
        corr_table_path = f"{stem1}/{stem1}_correlations.txt"
        output_files[corr_table_path] = corr_table_txt.encode("utf-8")
        print(f"  ✓ {corr_table_path}  ({len(output_files[corr_table_path]):,} bytes)")

    # Stage 6: Cross-group correlations (plain-text table)
    cross_txt      = _build_cross_group_txt(stem1, cross_corrs)
    cross_txt_path = f"{stem1}/_cross_group_correlations.txt"
    output_files[cross_txt_path] = cross_txt.encode("utf-8")
    print(f"  ✓ {cross_txt_path}  ({len(output_files[cross_txt_path]):,} bytes)")
...
def build_summary(data: dict, body_limit: int = 300,
    ...
    lines += [
        ...
        f"Actionable requests : {len(active_samplers)}",
    ...
    for i, s in enumerate(active_samplers, 1):
        ...
        lines.append(f"  Assert   : {asrt.get('response_code')}")
...
def process_folder(input_dir: Path, output_dir: Path,
    ...
    for sd in scenario_dirs:
        ...
        all_corr: list[dict] = []          # collect per-group corr data for combined table
        ...
        # Write one combined correlations table at the scenario parent directory
        scenario_out_dir = output_dir / scenario_name
        _write_combined_correlations_table(scenario_name, all_corr, scenario_out_dir)


def _build_combined_correlations_table(scenario_name: str,
                                        all_corr: list[dict]) -> str:
    """
    Build a combined correlations table for ALL groups in one scenario.

    *all_corr* is a list of per-group correlation dicts (loaded from
    correlations/*.json).  Each entry may be empty ({}) if a group has no
    correlations – those are silently skipped.

    Output (plain text) contains:

      TABLE 1 – Dynamic Variables
        One row per unique correlated variable, across every group.
        Columns:  #  |  Group  |  Variable  |  Captured Value  |
                  Src Sess  |  Src Method  |  Source URL  |
                  Response Field  |  Extractor Hint

      TABLE 2 – Correlation Map
        One row per source→destination link, across every group.
        Columns:  Group  |  Variable  |  Src Sess  |  Dst Sess  |
                  Dst Method  |  Destination URL  |  Destination Field
    """

    # Helper: format one table row by truncating each cell to its column width
    def _row(*cells, widths: dict) -> str:
        return "  " + "  |  ".join(
            str(c)[:w].ljust(w) for c, w in zip(cells, widths.values())
        )

    # Collect data from every group that has correlations
    dyn_rows:  list[tuple] = []   # (group, idx_global, dv)
    corr_rows: list[tuple] = []   # (group, corr, token_to_var)

    total_vars = 0
    total_links = 0
    global_idx = 0

    for corr_data in all_corr:
        if not corr_data:
            continue
        group     = corr_data.get("group", "?")
        dyn_vars  = corr_data.get("dynamic_variables", [])
        corr_list = corr_data.get("correlations", [])

        token_to_var = {
            dv["token_value"]: dv["variable_name"]
            for dv in dyn_vars if dv.get("token_value")
        }

        for dv in dyn_vars:
            global_idx += 1
            dyn_rows.append((group, global_idx, dv))
            total_vars += 1

        for c in corr_list:
            # Skip noisy HTML / full-URL correlations (mirrors _build_corr_maps)
            note = c.get("extraction_note", "")
            tok  = c.get("token_value", "")
            if "html." in note or "[@" in note:
                continue
            if tok.startswith("/") and ("?" in tok or "&" in tok):
                continue
            corr_rows.append((group, c, token_to_var))
            total_links += len(c.get("destinations", []))

    # ── Document header ───────────────────────────────────────────────────────
    lines = [
        f"=== COMBINED CORRELATIONS : {scenario_name} ===",
        f"Generated              : {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        f"Groups with correlations: {len([c for c in all_corr if c])}",
        f"Total dynamic variables : {total_vars}",
        f"Total destination links : {total_links}",
        "",
    ]
    ...
    lines += [
        "=" * 130,
        "  TABLE 1 — DYNAMIC VARIABLES  (what to extract and from where)",
        "=" * 130,
        "",
        _row("#", "Group", "Variable", "Captured Value", "Sess", "Method",
             "Source URL", "Response Field", "Extractor Hint", widths=C1),
        "  " + "-+-".join("-" * w for w in C1.values()),
    ]

    for group, idx, dv in dyn_rows:
        # Re-use the existing _extractor_hint already defined in Tool.py
        hint = _extractor_hint(
            dv.get("extraction_note", ""),
            dv.get("source_field", ""),
            dv.get("variable_name", ""),
        ...
        lines.append(_row(
            ...
            group,
            f"${{{dv['variable_name']}}}",
            str(dv.get("token_value", ""))[:36],
            str(dv.get("source_session", "?")),
            str(dv.get("source_method", "?")),
            str(dv.get("source_url", ""))[:52],
            str(dv.get("source_field", ""))[:32],
            hint[:58],
            widths=C1,
        ))
    ...
    C2 = {
        ...
        "Dst Method":       10,
        "Destination URL":  58,
        "Destination Field": 40,
    }

    lines += [
        "=" * 130,
        "  TABLE 2 — CORRELATION MAP  (where each variable is consumed)",
        "=" * 130,
        "",
        _row("Group", "Variable", "Src Sess", "Dst Sess", "Dst Method",
             "Destination URL", "Destination Field", widths=C2),
        "  " + "-+-".join("-" * w for w in C2.values()),
    ]

    for group, c, token_to_var in corr_rows:
        token    = c.get("token_value", "")
        var_name = token_to_var.get(token, token[:20])
        src_sid  = c.get("source", {}).get("session_id", "?")
        for dest in c.get("destinations", []):
            lines.append(_row(
                group,
                f"${{{var_name}}}",
...
def _write_combined_correlations_table(scenario_name: str,
                                        all_corr: list[dict],
                                        scenario_out_dir: Path) -> None:
    """
    Write the combined correlations table for *scenario_name* to:
        <scenario_out_dir>/{scenario_name}_correlations.txt

    Called once per scenario after all groups have been processed.
    Skips silently if none of the groups carry correlation data.
    """
    if not any(all_corr):
        return

    scenario_out_dir.mkdir(parents=True, exist_ok=True)
    out_path = scenario_out_dir / f"{scenario_name}_correlations.txt"
    out_path.write_text(
        _build_combined_correlations_table(scenario_name, all_corr),
        encoding="utf-8",
    )
    sz = out_path.stat().st_size
    print(f"  ✓ Combined correlations → {out_path}  ({sz:,} bytes)")
...
def extract_session(session: dict, skip_options: bool) -> dict | None:
    ...
    return {
        ...
        # ── Response Assertion (status code only) ────────────────────────────
        "assertion": {
...
def _compare_dicts(dict1, dict2, location, s1, cfg: Config, skip_keys=None):
    """
    Yield dynamic records for every key whose value differs between the two dicts.

    Minimum value length is intentionally low (1) here so that ALL changed
    values – including short numeric IDs, codes, and flags – are detected as
    dynamic fields.  Noise filtering happens later in extract_tokens where we
    have location context to make smarter decisions.
    """
    ...
    for key in set(dict1) | set(dict2):
        ...
        # Only skip truly empty values; let all non-empty changed values through.
        if v1 != v2 and v1:
...
def find_dynamic_values(pairs: list, cfg: Config) -> list:
    ...
    for s1, s2 in pairs:
        ...
        # ── Response body (JSON / XML / plain-text; skip binary/gzip) ───────
            ...
            if "json" in rct:
                ...
                if rj1 and rj2:
                    for rec in _compare_dicts(
                        ...
                        _add(rec)
                    _resp_body_matched = True
            if not _resp_body_matched and ("xml" in rct or rb1.strip().startswith("<")):
                ...
                if rx1 and rx2:
                    for rec in _compare_dicts(
                        ...
                        _add(rec)
                    _resp_body_matched = True
            # ── Plain-text / HTML fallback: if body changed but no structured
            #    parser succeeded, record the whole body as a single dynamic field
            #    so that find_correlations can still track the token.
            if not _resp_body_matched:
                if (len(rb1) >= cfg.min_token_len and
                        len(rb1) <= cfg.max_token_len):
                    _add({
                        "session_id": s1.get("session_id"),
                        "comment":    s1.get("comment", ""),
                        "method":     req1.get("method", ""),
                        "url":        req1.get("url", "")[:100],
                        "location":   "response.body(text)",
                        "field":      "body",
                        "value_run1": rb1,
                        "value_run2": rb2,
                    })
...
# Values that are always meaningless as correlation anchors regardless of where
# they appear – pure booleans, HTTP status text, empty markers, etc.
_NOISE_VALUES = {
    "", "true", "false", "null", "none", "undefined",
    "0", "1", "yes", "no", "on", "off",
    "get", "post", "put", "patch", "delete", "options", "head",
    "ok", "error", "success", "fail", "failed",
}


def _is_noise(val: str) -> bool:
    """Return True if val is too generic to be a useful correlation anchor."""
    if not val or val.lower() in _NOISE_VALUES:
        return True
    # Pure whitespace
    if not val.strip():
        return True
...
def extract_tokens(df: dict, cfg: Config) -> list:
    ...
    Correlation is NOT limited to long opaque tokens – ANY dynamic value
    (short IDs, numeric codes, GUIDs, form fields, …) that flows from a
    response into a subsequent request is a valid correlation candidate.

    Location-aware minimum lengths:
      • URL query params   → 2 chars  (param name gives context)
      • Body fields        → 2 chars  (field name gives context)
      • Response headers   → 4 chars  (header name gives context)
      • Cookies            → cfg.min_token_len (8) – cookie values are usually
                             meaningful only when substantial
      • Authorization      → cfg.min_token_len (8)
      • Default (other)    → 4 chars

    Values in _NOISE_VALUES are always skipped.
    Values longer than cfg.max_token_len are also skipped (binary noise).
    """
    ...
    if not val or len(val) > cfg.max_token_len:
        return tokens
    if _is_noise(val):
        return tokens
    ...
    if "cookie" in fname:
        for name, cval in parse_cookie_header(val).items():
            if len(cval) >= cfg.min_token_len and not _is_noise(cval):
    ...
    if fname == "authorization" and val.lower().startswith("bearer "):
        ...
        if len(bearer) >= cfg.min_token_len and not _is_noise(bearer):
    ...
    if fname == "location":
        try:
            for k, v in parse_qsl(urlsplit(val).query, keep_blank_values=True):
                if len(v) >= 2 and not _is_noise(v):
                    tokens.append((v, f"redirect_param[{k}]"))
        except Exception:
            pass
        if not tokens and len(val) >= 2 and not _is_noise(val):
            tokens.append((val, "location_url"))
        return tokens

    # ── URL query param – even short IDs are valid correlations ──────────────
    if "url.query" in loc:
        if len(val) >= 2:
            tokens.append((val, f"query[{df['field']}]"))
        return tokens

    # ── Body field (JSON / XML / form) – short IDs common here ───────────────
    if "body" in loc:
        if len(val) >= 2:
            tokens.append((val, f"body_field[{df['field']}]"))
        return tokens

    # ── Response / request headers – require at least 4 chars ────────────────
    if "headers" in loc:
        if len(val) >= 4:
            tokens.append((val, f"{loc}.{fname}"))
        return tokens

    # ── Default – whole value, at least 4 chars ───────────────────────────────
    if len(val) >= 4:
        tokens.append((val, f"{loc}.{fname}"))
    return tokens
...
def _token_in_blob(token: str, blob: str) -> bool:
    """
    Check whether *token* is meaningfully present in *blob*.

    For short tokens (≤ 7 chars) we require a word-boundary match so that
    a short ID like "42" doesn't match inside "1042" or "application/json".
    For longer tokens a plain substring search is sufficient.
    """
    if token not in blob:
        return False
    if len(token) > 7:
        return True
    # Short value: require it to appear as a standalone word / value
    # (surrounded by non-alphanumeric characters or at string boundary)
    import re as _re
    pattern = r'(?<![A-Za-z0-9_\-])' + _re.escape(token) + r'(?![A-Za-z0-9_\-])'
    return bool(_re.search(pattern, blob))


def _find_field_in(obj, token: str, prefix=""):
    """
    Return the first dotted JSON path in obj whose string value EXACTLY equals
    token (for short values ≤ 7 chars) or contains token (for longer values).

    Exact matching for short values prevents false positives where a short ID
    like "42" would otherwise match any string that happens to contain "42".
    """
    exact = len(token) <= 7
    if isinstance(obj, dict):
        for k, v in obj.items():
        ...
        if exact and obj == token:
            return prefix
        elif not exact and token in obj:
            return prefix
    elif not isinstance(obj, (dict, list)):
        # Handle numbers, bools stored as Python primitives
        if str(obj) == token:
            return prefix
    return None
...
def _find_in_response(res: dict, token: str) -> str | None:
    ...
    Checks in priority order:
      ...
      3. Body (XML/HTML-flattened if possible) → response.body(xml).<dotted.path>
      4. Raw body fallback → response.body
    ...
    if token in body:
        ...
        if body.strip().startswith("<"):
            x = try_parse_xml(body)
            if x:
                for path, val in x.items():
                    s_val = str(val)
                    match = (s_val == token) if len(token) <= 7 else (token in s_val)
                    if match:
                        return f"response.body(xml).{path}"
        # 4. Raw body fallback
...
def find_cross_group_correlations(all_sessions1: list,
                                   group_results: list,
                                   cfg: Config) -> list:
    """
    Cross-group correlation pass.

    After the per-group analysis is complete, collect every dynamic token found
    in ANY group and search for it across the FULL run-1 session list.  Only
    keep entries where:

      • The SOURCE group differs from at least one DESTINATION group  (i.e. the
        token crosses a group boundary — the whole point of this pass).
      • The correlation is not already covered by a per-group result.

    Returns a list of cross-group correlation records in the same dict shape as
    find_correlations(), augmented with:
        "source_group"       – group name of the response that produced the token
        "destination_groups" – sorted list of group names that consume the token

    These records are later merged into the per-group correlation files by
    save_cross_group_correlations().
    """
    if not group_results or not all_sessions1:
        return []

    # ── Build the global actionable session list in session_id order ──────────
    actionable_all = sorted(
        [s for s in all_sessions1 if _is_jmeter_actionable(s)],
        key=lambda s: s.get("session_id", 0),
    )

    # Quick lookup: session_id → group name
    sid_to_group: dict[int, str] = {}
    for grp, _dyn, _corrs in group_results:
        for s in all_sessions1:
            if s.get("comment", "") == grp:
                sid_to_group[s.get("session_id")] = grp

    # Collect all tokens already handled within their own group
    # Key: SHA-256(token) → set of group names already correlated in
    intra_seen: dict[str, set] = {}
    for grp, _dyn, corrs in group_results:
        for c in corrs:
            if not c.get("correlated"):
                continue
            h = hashlib.sha256(c["token_value"].encode()).hexdigest()
            intra_seen.setdefault(h, set()).add(grp)

    # Collect all unique dynamic tokens across every group (run-1 values only)
    candidate_tokens: list[tuple[str, str, str]] = []  # (token, extraction_note, source_group)
    seen_hashes: set = set()
    for grp, dyn_fields, _corrs in group_results:
        for df in dyn_fields:
            for token, extraction_note in extract_tokens(df, cfg):
                ...
                if h in seen_hashes:
                    continue
                seen_hashes.add(h)
                candidate_tokens.append((token, extraction_note, grp))

    cross_corrs: list[dict] = []

    for token, extraction_note, origin_group in candidate_tokens:
        token_hash = hashlib.sha256(token.encode()).hexdigest()

        # ── SOURCE: earliest actionable response (any group) containing token ──
        source = None
        for s in actionable_all:
            if _token_in_blob(token, _response_blob(s)):
        ...
        source_group = sid_to_group.get(source_sid, origin_group)

        # ── DESTINATIONS: all later actionable requests (any group) using token ─
        destinations: list[dict] = []
        dest_groups:  set[str]   = set()
        for s in actionable_all:
            sid = s.get("session_id")
            if sid <= source_sid:
                continue
            if _token_in_blob(token, _request_blob(s)):
                fp  = _find_in_request(s.get("request", {}), token)
                dg  = sid_to_group.get(sid, "?")
                destinations.append({
                    "session_id": sid,
                    "comment":    s.get("comment", ""),
                    "method":     s.get("request", {}).get("method", ""),
        ...
        # Keep only if the token crosses AT LEAST ONE group boundary
        cross_dest_groups = dest_groups - {source_group}
        if not cross_dest_groups:
            continue

        # Skip if this exact token is already fully covered by intra-group results
        # for EVERY destination group it touches
        already_covered = intra_seen.get(token_hash, set())
        new_groups = dest_groups - already_covered
        if not new_groups:
            continue

        cross_corrs.append({
            "dynamic_field_location": "cross_group",
            "dynamic_field_name":     extraction_note,
            "extraction_note":        extraction_note,
            "token_value":            token,
            ...
            "destination_groups":     sorted(dest_groups),
            "cross_groups":           sorted(cross_dest_groups),
            "correlated":             True,
            "detected_by":            "cross_group",
        })

    return cross_corrs


def save_cross_group_correlations(cross_corrs: list,
                                   file_a, file_b,
                                   correlations_dir: Path) -> None:
    """
    Merge cross-group correlation records into the relevant per-group
    correlation JSON files that already exist under *correlations_dir*.

    For each cross-group correlation:
      • The SOURCE group's file gets a new entry where destinations include
        ONLY the cross-group destinations (group != source_group).
      • Each DESTINATION group's file gets the same entry appended so that
        the agent processing that group knows it must parameterize the token.

    A separate  _cross_group_correlations.json  summary is also written to
    *correlations_dir* listing all cross-group entries in one place.
    """
    if not cross_corrs:
        print("  –  No cross-group correlations found.")
        return

    correlations_dir = Path(correlations_dir)
    correlations_dir.mkdir(parents=True, exist_ok=True)

    # Load all existing per-group JSON files into memory
    group_files: dict[str, Path] = {}      # group_name → path
    group_data:  dict[str, dict] = {}      # group_name → loaded dict
    for jf in sorted(correlations_dir.glob("*.json")):
        ...
        try:
            ...
            gname = doc.get("group", "")
            if gname:
                group_files[gname] = jf
                group_data[gname]  = doc
        except Exception:
            pass

    added_per_group: dict[str, int] = defaultdict(int)

    for c in cross_corrs:
        source_grp = c.get("source_group", "")
        all_dests  = c.get("destinations", [])

        # Groups that appear in destinations (excluding source)
        dest_groups = set(d.get("group", "") for d in all_dests) - {source_grp}

        # ── Append to source group file ─────────────────────────────────────
        for grp in ([source_grp] + list(dest_groups)):
            if not grp or grp not in group_data:
                continue
            doc = group_data[grp]

            # Build a scoped copy with only relevant destinations for this group
            if grp == source_grp:
                scoped_dests = [d for d in all_dests if d.get("group", "") != source_grp]
            else:
                scoped_dests = [d for d in all_dests]

            entry = {
                "dynamic_field_location": c["dynamic_field_location"],
                "dynamic_field_name":     c["dynamic_field_name"],
                "extraction_note":        c["extraction_note"],
                "token_value":            c["token_value"],
                "token_preview":          c["token_preview"],
                "token_length":           c["token_length"],
                "source":                 c["source"],
                "source_group":           source_grp,
                "destinations":           scoped_dests,
                "cross_groups":           c["cross_groups"],
                "correlated":             True,
                "detected_by":            "cross_group",
            }

            # Avoid exact-token duplicates within this group file
            existing_tokens = {x.get("token_value") for x in doc.get("correlations", [])}
            if c["token_value"] not in existing_tokens:
                doc.setdefault("correlations", []).append(entry)
                # Also append to dynamic_variables if not already present
                existing_vars = {
                    dv["token_value"]
                    for dv in doc.get("dynamic_variables", [])
                }
                if c["token_value"] not in existing_vars:
                    src = c["source"]
                    doc.setdefault("dynamic_variables", []).append({
                        "variable_name":   _derive_var_name(c["extraction_note"],
                                                            c["dynamic_field_name"]),
                        "field_name":      c["dynamic_field_name"],
                        "extraction_note": c["extraction_note"],
                        "token_value":     c["token_value"],
                ...
                doc["summary"]["correlations_found"] = len(
                    [x for x in doc["correlations"] if x.get("correlated")]
                )
                added_per_group[grp] += 1

    # ── Write back updated per-group files ───────────────────────────────────
    for grp, doc in group_data.items():
        if added_per_group.get(grp, 0) > 0:
            jf = group_files[grp]
            with open(jf, "w", encoding="utf-8") as f:
                json.dump(doc, f, indent=2, ensure_ascii=False)
            print(f"  ✓ [{grp}] +{added_per_group[grp]} cross-group entries → {jf.name}")

    # ── Write combined cross-group summary file ───────────────────────────────
    summary_path = correlations_dir / "_cross_group_correlations.json"
    summary_doc  = {
        "run1": str(file_a),
        "run2": str(file_b),
        "generated": datetime.now().isoformat(),
        "summary": {
            "cross_group_correlations": len(cross_corrs),
            "groups_affected":          sorted(
                {c["source_group"] for c in cross_corrs}
                | {g for c in cross_corrs for g in c.get("cross_groups", [])}
            ),
        },
        "cross_group_correlations": cross_corrs,
    }
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(summary_doc, f, indent=2, ensure_ascii=False)
    print(f"  ✓ Cross-group summary → {summary_path}  ({len(cross_corrs)} entries)")


def _derive_var_name(extraction_note: str, field_name: str) -> str:
    """Derive a clean JMeter variable name from extraction_note / field_name."""
    m = re.search(r'cookie\[([^\]]+)\]', extraction_note)
    if m:
    ...
    if m:
        return re.sub(r'\W+', '_', m.group(1)).strip("_")
    if extraction_note == "bearer_token":
        return "bearer_token"
    if extraction_note.startswith("body_field["):
        inner = re.sub(r'^body_field\[', '', extraction_note).rstrip("]")
        return re.sub(r'\W+', '_', inner.split(".")[-1]).strip("_") or "dynamic_var"
    return re.sub(r'\W+', '_', field_name.split(".")[-1]).strip("_") or "dynamic_var"
    ...
    for df in dynamic_fields:
        for token, extraction_note in extract_tokens(df, cfg):
            ...
            for session in actionable:
                if _token_in_blob(token, _response_blob(session)):
            ...
            for session in actionable:
                ...
                if sid <= source_sid:
                    continue
                if _token_in_blob(token, _request_blob(session)):
...
# ─── Structural correlation – ASP.NET hidden form fields ─────────────────────

# Hidden fields that ASP.NET regenerates every request.
# The *value* is always too large / too opaque for token-value matching,
# so we use structural (name-pattern) detection instead.
_ASPNET_HIDDEN_FIELDS = {
    ...
    "__eventtarget",
    "__eventargument",
    "__requestverificationtoken",   # also used by MVC anti-forgery
}

# Regex that matches an HTML hidden input whose name is one of the above.
# Captures: field_name, field_value (may be empty for compressed VIEWSTATE).
_HIDDEN_INPUT_RE = re.compile(
    r'<input[^>]+type=["\']?hidden["\']?[^>]*>|'
    r'<input[^>]+name=["\']?(__[A-Za-z]+)["\']?[^>]*>',
    re.IGNORECASE,
)
_HIDDEN_VALUE_RE = re.compile(
    r'<input\b[^>]*\bname=["\']?(__[A-Za-z]+)["\']?[^>]*\bvalue=["\']?([^"\'>\s]*)["\']?[^>]*/?>',
    re.IGNORECASE,
)


def _extract_hidden_fields_from_html(body: str) -> dict:
    """Return {field_name_lower: value} for ASP.NET hidden inputs found in body."""
    fields = {}
    for m in _HIDDEN_VALUE_RE.finditer(body):
        name = m.group(1).lower()
        val  = m.group(2)
        if name in _ASPNET_HIDDEN_FIELDS:
            fields[name] = val
    return fields


def find_aspnet_form_correlations(sessions: list,
                                   existing_correlations: list) -> list:
    """
    Structural detection of ASP.NET hidden form fields (__VIEWSTATE,
    __EVENTVALIDATION, __RequestVerificationToken, etc.).

    These values are always session-unique and far too large for token-value
    matching (max_token_len would filter them).  Instead we use name-pattern
    matching:

      SOURCE  – the most recent prior actionable response whose HTML body
                contains  name="__VIEWSTATE"  (or the specific field name).
      DEST    – every actionable POST request that sends that field in its
                form body.

    Returns only NEW correlation records (deduplicated against
    *existing_correlations* by (field_name, dest_session_id) pair).
    """
    existing_keys = {
        (c.get("dynamic_field_name", "").lower(), d.get("session_id"))
        for c in existing_correlations
    ...
    # Build a map: session_id → list of hidden field names produced by that response
    response_hidden: dict = {}   # sid → {field_name_lower: value}
    for s in actionable:
        body = (s.get("response", {}).get("body", "") or "")
        if not body or is_binary(body):
            continue
        if "<input" not in body.lower():
            continue
        fields = _extract_hidden_fields_from_html(body)
        if fields:
            response_hidden[s.get("session_id")] = (s, fields)

    # For each actionable POST with hidden form fields, find the source response
    seen_field_source: dict = {}   # field_name → last known source session
    ...
    for s in actionable:
        ...
        if req.get("method", "").upper() != "POST":
            continue
        body = req.get("body", "") or ""
        ct   = req.get("headers", {}).get("Content-Type", "").lower()
        if "form" not in ct and not ("=" in body and "&" in body):
            continue

        form_fields = parse_form_body(body)
        post_sid    = s.get("session_id")

        for field_name, field_value in form_fields.items():
            if field_name.lower() not in _ASPNET_HIDDEN_FIELDS:
                continue
            if not field_value:
                continue

            # Find the most recent prior response that produced this field
            source_session = None
            source_field_val = None
            for src_sid, (src_session, src_fields) in sorted(
                    response_hidden.items(), key=lambda x: x[0]):
                if src_sid >= post_sid:
                    break
                if field_name.lower() in src_fields:
                    source_session   = src_session
                    source_field_val = src_fields[field_name.lower()]

            if not source_session:
                # No HTML source found – still record dest-only correlation
                # so the summary shows a parameterisation need.
                source_entry = None
            else:
                source_entry = {
                    "session_id": source_session.get("session_id"),
                    "comment":    source_session.get("comment", ""),
                    "method":     source_session.get("request", {}).get("method", ""),
                    "url":        source_session.get("request", {}).get("url", "")[:100],
                    "field":      f"response.body(html)[name={field_name}]",
                    "value":      source_field_val or "(large/compressed)",
                }
            ...
            dest_entry = {
                ...
                "method":     req.get("method", ""),
                "url":        req.get("url", "")[:100],
                "field":      f"request.body(form).{field_name}",
            }

            key = (field_name.lower(), post_sid)
            if key in existing_keys:
                continue
            existing_keys.add(key)

            # Check if we already have a correlation record for this field
            # (e.g. from a previous POST in the same group) – if so, append dest
            merged = False
            for c in new_corrs:
                if (c.get("dynamic_field_name", "").lower() == field_name.lower()
                        and c.get("source") == source_entry):
                    c["destinations"].append(dest_entry)
                    c["correlated"] = True
                    merged = True
                    break
            ...
            if not merged:
                new_corrs.append({
                    "dynamic_field_location": "request.body(form)",
                    "dynamic_field_name":     field_name,
                    "extraction_note":        f"aspnet_hidden[{field_name}]",
                    "token_value":            field_value[:100] + ("..." if len(field_value) > 100 else ""),
                    "token_preview":          field_value[:100] + ("..." if len(field_value) > 100 else ""),
                    "token_length":           len(field_value),
                    "source":                 source_entry,
                    "destinations":           [dest_entry],
                    "correlated":             source_entry is not None,
                    "detected_by":            "aspnet_structural",
                })
...
def group_sessions_by_comment(sessions: list) -> dict:
    ...
    Sessions with an empty/missing comment are silently skipped –
    they have no user-defined label and should not produce output folders.
    Returns an OrderedDict preserving insertion order.
    """
    groups = defaultdict(list)
    for s in sessions:
        key = s.get("comment", "").strip()
        if not key:          # skip uncommented sessions entirely
            continue
        groups[key].append(s)
    return dict(groups)
...
def correlate_only(file_a, file_b, grouped_root=None,
    ...
    for grp in all_groups:
        ...
        structural     = find_structural_correlations(g1, correlations, cfg)
        aspnet         = find_aspnet_form_correlations(g1, correlations + structural)
        correlations   = correlations + structural + aspnet
    ...
    # ── Cross-group correlation pass ──────────────────────────────────────────
    # Find tokens that flow from one group's response into another group's request
    _log("Running cross-group correlation pass …")
    cross_corrs = find_cross_group_correlations(sessions1, group_results, cfg)
    _log(f"  Cross-group correlations found: {len(cross_corrs)}")
    ...
    if cross_corrs:
        print(THIN)
        print(f"  {'[cross-group]':<25} {'':>8} {len(cross_corrs):>6} {len(cross_corrs):>6}")
    print(SEP)
    ...
    if output_report:
        save_report(None, None, file_a, file_b, output_report,
                    ...
                    correlations_dir=correlations_dir)
        # Merge cross-group entries into per-group files AFTER save_report
        # so the per-group files already exist on disk
        if cross_corrs and correlations_dir:
            _log("Saving cross-group correlation entries …")
            save_cross_group_correlations(cross_corrs, file_a, file_b,
                                          Path(correlations_dir))
...
def analyze(file_a=None, file_b=None,
    ...
    for grp in all_groups:
        ...
        correlations = correlations + structural + aspnet
        real = sum(1 for c in correlations if c["correlated"])
        _log(f"     Correlations   : {len(correlations)}  (real: {real}, structural: {len(structural)}, aspnet: {len(aspnet)})")
