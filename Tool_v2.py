...
class SazSummaryTool(BaseTool):
    ...
    def _run(
        ...
        try:
            ...
            saz_bytes_1 = _download_bytes(raw_url_1, token)
            print(f"[SazSummaryTool] Downloading {saz_name_2} …")
            saz_bytes_2 = _download_bytes(raw_url_2, token)
            ...
            for rel_path, content_bytes in sorted(summary_files.items()):
                repo_path      = f"{folder_name}/{rel_path}"
                commit_message = f"Add SAZ summary: {Path(rel_path).name}"
...
def _run_pipeline_in_memory(
    ...
    """
    # Stage 1: Parse SAZ bytes → session dicts
    print("[Pipeline] Stage 1 – Parsing SAZ bytes …")
    data1     = _parse_saz_bytes(saz_bytes_1, saz_name_1)
    data2     = _parse_saz_bytes(saz_bytes_2, saz_name_2)
    ...
    # Stage 4: JMeter data extraction + summary generation (in memory)
