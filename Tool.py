...
class SazSummaryTool(BaseTool):
    ...
    def _run(
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="saz_tool_") as tmp:
                tmp_path = Path(tmp)
                saz_dir  = tmp_path / "saz"
                saz_dir.mkdir()
                ...
                local_saz_1 = saz_dir / saz_name_1
                local_saz_2 = saz_dir / saz_name_2

                print(f"[SazSummaryTool] Downloading {saz_name_1} …")
                _download_file(raw_url_1, local_saz_1, token)
                print(f"[SazSummaryTool] Downloading {saz_name_2} …")
                _download_file(raw_url_2, local_saz_2, token)

                print("[SazSummaryTool] Running pipeline …")
                work_dir = tmp_path / "pipeline"
                work_dir.mkdir()
                summary_files = _run_pipeline(local_saz_1, local_saz_2, work_dir)
                ...
                uploaded_urls    = []
                agent_input_root = work_dir / "agent_input"

                for sf in sorted(summary_files):
                    try:
                        rel = sf.relative_to(agent_input_root)
                    except ValueError:
                        rel = Path(sf.name)

                    repo_path      = f"{folder_name}/{rel.as_posix()}"
                    commit_message = f"Add SAZ summary: {rel.name}"

                    print(f"[SazSummaryTool] Uploading → {repo_path}")
                    html_url = _upload_file_to_github(
                        api_base=api_base,
                        branch=branch,
                        repo_path=repo_path,
                        local_path=sf,
